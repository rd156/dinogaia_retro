...
const CombatPage: React.FC = () => {
  ...
  useEffect(() => {
    const createCombat = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/fight/fast/${fightId}`, {
          method: "GET",
          headers: {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/fight/fast/update/${fightId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (data.fight_id)
      {
        console.log(data.fight_id)
        window.location.href = "/fight/fast/preview/" + data.fight_id  
      }
      else
      {
